...
(defn -main
  ...
  (let [conn (-> "jdbc:mysql://localhost:3306/test?user=root"
    ...
    (println )
    (println "--------------------------------------------------------")
    (println "CASE 3: broken! -> immutant, WITHOUT mysql dep in uberjar")
    (println)
    (println "lein with-profiles immutant uberjar \\")
    (println " && java -jar target/abc-0.0.1-SNAPSHOT-standalone.jar")
    (println)
    (println "This will print an exception that no suitable driver")
    (println "could be found! What going on here?")
    (println)
    (println "#########################################################")
    (println)
